...
export default class TinkoffASDKCore {
	...
	private request<T extends {request:any,response:any}>(params: T['request'], method: T.Method, arrayOfParams: string[]): Promise<T['response']> {
	 ...
	 * 
	 */
	Init(params: T.Init['request']): Promise<T.Init['response']> {
		return this.request<T.Init>(params, 'Init', ['CustomerKey','OrderId','Amount'])
	}
	/**
	 * 
	 */
	Finish(params: T.Finish['request']): Promise<T.Finish['response']> {
		return this.request<T.Finish>(params, 'Finish', ['PaymentId', 'PaymentSource'])
	}
