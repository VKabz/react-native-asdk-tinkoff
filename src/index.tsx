...
export default class TinkoffASDKCore {
  ...
  Pay(params: Init['request']): Promise<Init['response']> {
    return NativeModules.AsdkTinkoff.Pay(
      JSON.stringify({
        ...this.config,
        ...params,
      })
  ...
  ApplePayAvailable(merchant: String): Promise<boolean> {
    ...
    return NativeModules.AsdkTinkoff.ApplePayAvailable(
      JSON.stringify({
        ...this.config,
      }),
      merchant
  ...
  ApplePay(
    params: Init['request'],
    merchant: String
  ): Promise<Init['response']> {
    if (Platform.OS !== 'ios') {
      throw new Error(`Cannot use ApplePay on ${Platform.OS}`);
    }
    return NativeModules.AsdkTinkoff.ApplePay(
      JSON.stringify({
        ...this.config,
        ...params,
      }),
  ...
  GooglePay(params: Init['request']): Promise<Init['response']> {
    ...
    return NativeModules.AsdkTinkoff.GooglePay(
      JSON.stringify({
        ...this.config,
        ...params,
      })
